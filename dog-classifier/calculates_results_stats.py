...
def calculates_results_stats(results_dic):
    ...
    # Declare an empty dictionary and other variables
    results_stats_dic = {}
    ...
    # Loop to calculate all above variables
    for result in results_dic.values():
        # Calculate dogs images
        if result[3] == 1:
            n_dogs_img += 1
        # Otherwise, Calculate not dogs images
        else:
            n_notdogs_img += 1
        # Calculate count match labels
        if result[2] == 1:
            n_match += 1
        # Calculate count of correct dogs label match
        if result[3] == 1 and result[4] == 1:
            n_correct_dogs += 1
        # Calculate count of correcnt non-dogs label match
        if result[3] == 0 and result[4] == 0:
            n_correct_notdogs += 1
        # Calculate count of correct dogs breed label match
    ...
    # Calculate Percentage Label Matches ( regardless if they're a dog)
